...
impl Nesting {
  pub fn active(&self) -> Option<&[Box<dyn RtorIface>]> {
    if let Some(cursor) = self.cursors.last() && *cursor < self.all.len() {
      Some(&self.all[*cursor..])
    } else {
      None
    }
  }
  pub fn start_consumer(&mut self) {
    self.cursors.push(self.all.len());
  }
  pub fn stop_consumer(&mut self) {
    self.cursors.pop();
  }
  pub fn start_producer(&mut self, producer: Box<dyn RtorIface>) {
    self.all.push(producer);
  }
